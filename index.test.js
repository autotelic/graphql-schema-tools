...
test(
  ...
  async ({ is }) => {
    const { source: sourceOneResult } = normalizeGQLSource(sourceOne)
    const { source: sourceTwoResult } = normalizeGQLSource(sourceTwo)
...
test(
  'should provide a snippet of where a parse error occurs.',
  async ({ is }) => {
    const schemaWithErrorOne = `
      type Query foo: String
      `

    const schemaWithErrorTwo = `
      type Query {
        foo(): String
      }`

    const { error: errorOne } = normalizeGQLSource(schemaWithErrorOne)
    const { error: errorTwo } = normalizeGQLSource(schemaWithErrorTwo)

    is(errorOne.message, 'Syntax Error: Unexpected Name "foo". Found near: `Query foo:`.')
    is(errorTwo.message, 'Syntax Error: Expected Name, found ")". Found near: `{ foo():`.')
  })

test(
  'should throw an Error if not passed a string',
  async ({ throws }) => {
    throws(() => normalizeGQLSource(), Error('gql-schema-normalizer: Expected a string.'))
  })

...
test(
  ...
  async ({ equal }) => {
    ...
    equal(sourceOneResult, sourceTwoResult, 'once normalized, schemas should be exactly the same if there were only stylistic differences previously.')
    equal(sourceOneResult, expected)
    equal(sourceTwoResult, expected)
    equal(sourceOneError, undefined)
    equal(sourceTwoError, undefined)
  })
...
test(
  ...
  async ({ equal }) => {
    ...
    equal(sourceOneResult, sourceTwoResult, 'once normalized, schemas should be exactly the same if there were only stylistic differences previously.')
    equal(sourceOneResult, minifyGQLSource(expected))
    equal(sourceTwoResult, minifyGQLSource(expected))
    equal(sourceOneError, undefined)
    equal(sourceTwoError, undefined)
  })
...
test(
  ...
  async ({ equal }) => {
    ...
    equal(error.message, 'Syntax Error: Expected Name, found ")". Found near: `Query { foo(): String`.')
  })
